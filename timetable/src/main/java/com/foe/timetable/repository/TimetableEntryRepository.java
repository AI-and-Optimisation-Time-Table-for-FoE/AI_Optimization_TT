...
public interface TimetableEntryRepository extends JpaRepository<TimetableEntry, Integer> {
    ...
    @Query(value = "SELECT DISTINCT te.* FROM timetable_entry te " +
                   "JOIN batch_module bm ON te.batch_module_id = bm.batch_module_id " +
                   "LEFT JOIN batch_module_lecturer bml ON bm.batch_module_id = bml.batch_module_id " +
                   "WHERE bm.lecturer_id = :lecturerId OR bml.lecturer_id = :lecturerId", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query("DELETE FROM TimetableEntry te WHERE te.batchModule.batchModuleId = :batchModuleId")
    void deleteByBatchModuleId(@Param("batchModuleId") Integer batchModuleId);

    List<TimetableEntry> findByTimetableId(Integer timetableId);
    List<TimetableEntry> findByBatchModule_BatchModuleId(Integer batchModuleId);
}
