...
public class TimetableViewController {
    ...
    public ResponseEntity<List<TimetableEntryViewDto>> viewTimetableByBatch(
            @PathVariable int batchId,
            @org.springframework.web.bind.annotation.RequestParam(required = false) Integer departmentId) {
        List<TimetableEntryViewDto> schedule = timetableQueryService.getTimetableByBatchId(batchId, departmentId, true);
        if (schedule.isEmpty()) {
