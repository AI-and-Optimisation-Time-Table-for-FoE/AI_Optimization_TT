    ...
    <>
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        <div className="sidebar-header">
          <Link href={user ? `/${user.role}` : "/"} onClick={closeSidebar} className="sidebar-logo" style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <img 
              src="/logo.jpg" 
        ...
        {user && (
          <div style={{ padding: "16px", borderTop: "1px solid rgba(255, 255, 255, 0.1)" }}>
            <Link href="/profile" onClick={closeSidebar} style={{ display: "flex", alignItems: "center", gap: "10px", textDecoration: "none", color: "white", padding: "8px", borderRadius: "var(--radius-sm)", background: "rgba(255, 255, 255, 0.05)", transition: "background 0.2s", minWidth: 0 }}>
              {user.profilePicture ? (
                <img 
