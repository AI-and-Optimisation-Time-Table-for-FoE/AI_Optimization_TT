...
def calculate_fitness(schedule: List[TimetableEntryOutput], req: OptimizeRequest) -> float:
    ...
    for mod in req.modules:
        if mod.lecturerIds:
            lec_map[mod.batchModuleId] = list(mod.lecturerIds)
        else:
            lec_map[mod.batchModuleId] = [mod.lecturerId]
        
        name = mod.moduleName.lower()
        code = mod.moduleCode.lower()
        is_elective = "(te)" in name or "elective" in name or "(e)" in name or "te" in code
        elective_map[mod.batchModuleId] = is_elective
    ...
    for entry in schedule:
        ...
        # H2: Batch double-booking (no two modules can share a time slot)
        ...
        lec_ids = lec_map.get(entry.batchModuleId, [])
        ...
        # H13: Friday after 4:30 PM (Friday 16:30 onwards is blocked)
    ...
    # H11: 2-Credit, 3-Credit (3-Hour) and 4-Credit (4-Hour) Split Constraint Check
    ...
    # ── S5: Avoid lectures after 4:30 PM (max 1 day allowed, penalise excess heavily)
    late_count = 0
    wed_late_count = 0
    for entry in schedule:
        t = slot_map.get(entry.slotId)
    ...
    late_penalty = 0.0
    if late_count > 0:
        # Base penalty for having any late lectures
        late_penalty += late_count * 10.0
        # Extreme penalty for having more than 1 late lecture
        if late_count > 1:
            late_penalty += (late_count - 1) * 200.0
    if wed_late_count > 0:
        # Very strong penalty to avoid Wednesday late slot
        late_penalty += wed_late_count * 500.0
